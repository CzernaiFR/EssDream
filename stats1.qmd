...
Il classe les données sur une échelle de $0$ à $1$ et, en multipliant par $1\,000$, on obtient une série.

L'avantage est de ne pas avoir une suite de $0$ à $x$, et la différence entre les occurrences moindres et les occurrences élévées est clairement marquée.

On peut interpréter ça autrement. Il y a $9$ données. Au lieu de $1000$, multiplier par $9$.
...
Ou, en étant plus pragmatique, voire puriste, en multipliant par $8$ et en ajoutant $1$.
...
Essai avec les sorties totales des numéros de 1 à 40 :
...
Ça fonctionne. J'achète.

### Déroulement des opérations

Là, dimanche 23 mars 2026, on a $250$ tirages. Pour $R$, `round(250 / 3)` fait `r round(250 / 3)`. Et $3 * 83 = 249$. Il faudra en tenir compte. 

Jusqu'ici, on a un tableau `nums` et un tableau `dreams` qui contiennent les situations des numéros et des numéros `dreams` après les $83$ premiers tirages. Pour le tiers suivant des tirages, du tirage $84$ au tirage $166$, on va :

  - récupérer les numéros sortis ;
  - récupérer les variables concernant ces numéros ;
