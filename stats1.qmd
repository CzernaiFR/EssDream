...
La FDJ met à disposition des joueurs[^1] les résultats de **TOUS** ses jeux.
...
Une fois dézippé, c'est un fichier `.csv`[^2] qui contient $32$ colonnes, dont la grande majorité sont inutiles pour les statistiques (nombre de gagnants, montants des gains et autres joyeusetés inutiles).

[^2]: [ce qu'est un fichier `.csv`](https://fr.wikipedia.org/wiki/Comma-separated_values)
...
À noter que les tirages sont classés en ordre descendant, les plus récents d'abord. Un peu de nettoyage s'impose. Abréger les noms.
...
On va tranformer `jt` en facteur[^3]

[^3]: [ce qu'est un facteur en $R$](https://bookdown.org/ael/rexplor/chap3.html)
...
Pour les écarts, c'est « presque » pareil. Tous les écarts sont à $0$ au départ[^4].
...
nums |> 
  dplyr::count(so) |>
  mutate(prop = proportions(n) * 100) |> arrange(prop)
```
```{r}
sum(nums$so) / 40
summary(nums)
